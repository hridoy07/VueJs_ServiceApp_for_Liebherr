<template>
   <div name="registered_area">
    <img alt="Vue logo" src="../assets/logo.png" />
    <br><br>
    <h1 class="product">{{$t('message.nav2')}}</h1>
    <br>
    <nav>
    <!-- Navigation elements with vue-loop -->
      <b-nav ref="bnav1" pills small align="center">
        <b-nav-item 
          ...
          class="bnav"
        >{{$t('message.reg'+item.id)}}</b-nav-item>
      </b-nav>
    </nav>
  </div>
</template>
...
export default {
  name: "registered_area", //this is the name of the component
  data () {
   return { 
    items: [
    { id: 1, to: translation.i18nRoute({name:'login'}), text: "Login" },
    { id: 2, to: translation.i18nRoute({name:'register'}), text: "Register" },
    ]
   };
  },
